...
export class Mesh {
  ...
  constructor(
    private topicsService: RpcClientModule<CommsServiceDefinition, any>,
    ...
    this.listeners.push(
      listenPeerMessage(topicsService, `${this.peerId}.candidate`, this.onCandidateMessage.bind(this)),
      listenPeerMessage(topicsService, `${this.peerId}.offer`, this.onOfferMessage.bind(this)),
      listenPeerMessage(topicsService, `${this.peerId}.answer`, this.onAnswerListener.bind(this))
    )
  }
  ...
  public async connectTo(peerId: string, reason: string): Promise<void> {
    ...
    await this.topicsService.publishToTopic({
      topic: `${peerId}.offer`,
