...
export type P2PConfig = {
  ...
  peerId: string
  logger: ILogger
  topicsService: RpcClientModule<CommsServiceDefinition, any>
...
export class PeerToPeerAdapter implements MinimumCommunicationsAdapter {
  ...
  constructor(private config: P2PConfig, peers: Map<string, Position3D>) {
    ...
    this.mesh = new Mesh(this.config.topicsService, this.config.peerId, {
  ...
  private sendPacketToPeer(peer: string, payload: Uint8Array) {
    try {
      if (this.mesh.sendPacketToPeer(peer, payload)) {
        // MIGRATE: incrementCommsMessageSent(payload.length)
      }
    } catch (e: any) {
  ...
  private distanceTo(peerId: string) {
    const p = this.config.positionReader()
    const position: Position3D = [p.positionX, p.positionY, p.positionZ]
