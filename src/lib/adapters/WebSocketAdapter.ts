...
import { CommsIdentity, VoiceHandler } from '../types'
...
export class WebSocketAdapter implements MinimumCommunicationsAdapter {
  ...
  constructor(public url: string, private identity: CommsIdentity, public logger: ILogger) {}
  ...
  async connect(): Promise<void> {
    ...
    ws.onerror = (event) => {
      this.logger.error('socket error', event as any)
  ...
  private sendTopicMessage(reliable: boolean, topicMessage: rfc4.Packet) {
