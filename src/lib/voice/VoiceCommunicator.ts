...
export class VoiceCommunicator {
  ...
  onDataFrame: (frame: rfc4.Voice) => void = () => {}

  private loopbackElement: HTMLAudioElement = new Audio()

  constructor(private options: VoiceCommunicatorOptions) {
  ...
  async setInputStream(stream: MediaStream) {
    if (this.input) {
      ...
      if (this.input.recordingContext[0] !== this.context) {
        this.input.recordingContext[0].close().catch((e) => console.error("Error closing recording context", e))
      }
    }
  ...
  private createRTCLoopbackConnection(currentRetryNumber: number = 0): {
    ...
    ;(async () => {
      ...
      dst.ontrack = (e) => (this.loopbackElement.srcObject = e.streams[0])
      ...
      console.error("Error creating loopback connection", e)
      src.close()
      dst.close()
    })
  ...
  private async createOutput(src: string, relativePosition: VoiceSpatialParams) {
    ...
    readEncodedBufferLoop().catch((e) => console.log("Error while reading encoded buffer of " + src, e))
  }
  ...
  private createContext(contextOptions?: AudioContextOptions): AudioContextWithInitPromise {
    ...
    if (aContext.audioWorklet) {
      const workletInitializedPromise = aContext.audioWorklet
        ...
        .catch((e) => console.error("Error loading worklet modules: ", e))
      ...
      console.error("Error loading worklet modules: audioWorklet undefined")
